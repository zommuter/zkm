...
- [ ] **N-progress: amender phase progress reporting.** `zkm status` shows 100% once the conversion loop finishes, but NER (and any amender) runs silently after — no progress updates. zkm-ner should call `RunSession.tick()` (or equivalent) per document so `zkm status` reflects amender work in flight.
