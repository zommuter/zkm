...
- [ ] **[embed] Resume + complete the schema-3 embed rebuild (gated on [15b2]).** **RUNNING** as of 2026-05-28 (PID 95944, `zkm index`). **FAILED 2026-05-29** — oversized chunks + fatal timeout; root-caused in `docs/meeting-notes/2026-05-29-0859-embed-oversized-chunk-timeout.md`. **Gated on [95b9]+[de33]+[a600] (embed fixes) + [8497] (html.unescape fix, prevents new garbage) + [d1f2] (scrub, cleans existing corpus).** Decision (2026-05-21): feature-first single clean pass; see `docs/meeting-notes/2026-05-21-1908-embed-rebuild-eta-decision.md`. Kill PID 95944 before restarting. <!-- id:402c -->
...
- [ ] **[embed] Bump `_STORE_SCHEMA_VERSION` 3→4** (`src/zkm/embed.py:36`), bundled with [95b9]+[de33]. Contract: v3 stores re-embed from zero on next `zkm index`. — see `docs/meeting-notes/2026-05-29-0859-embed-oversized-chunk-timeout.md` <!-- id:a600 -->
- [ ] **[embed/ops] After [95b9]+[de33]+[a600]+[d1f2] land:** kill PID 95944, run `zkm scrub zkm-eml`, then `zkm index` clean; verify Riku's gate (zero oversized/split lines in first ~500 chunks) + `zkm doctor` embed docs == md count. Folds into `[402c]`/`[2c6e]`. — see `docs/meeting-notes/2026-05-29-0859-embed-oversized-chunk-timeout.md` <!-- id:30f5 -->
...
- [ ] **[M] Fix `html_to_markdown()` entity decoding so existing `quote_strip=True` fires on HTML emails** (`plugins/zkm-eml/`): `html_to_markdown()` leaves `&gt;`/`&nbsp;` undecoded so `quote_strip.find_tail_quote()` never sees real `>` chars — quoted tails survive into NER as garbage `org` entities. Bug fix in an already-shipped feature (not new scope). After fix: re-convert affected mails cleans frontmatter at source, reducing reliance on [d1f2] scrub. — see `docs/meeting-notes/2026-05-29-0859-embed-oversized-chunk-timeout.md` <!-- id:8497 -->
- [ ] **[M] Extend `zkm scrub zkm-eml`** (`plugins/zkm-eml/convert.py:497-555`): add HTML-entity-run pattern (`&gt;`/`&nbsp;`/`&amp;`-dominated values) alongside `_BASE64_FRAGMENT_RE`; zkm-eml minor bump. Contract: removes `&gt;&nbsp;` quoted-reply garbage entities (65/75 type `org` in e.g. `回复-回复-loan.md`). — see `docs/meeting-notes/2026-05-29-0859-embed-oversized-chunk-timeout.md` <!-- id:d1f2 -->
