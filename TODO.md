...
- [ ] **Stale dev-plugin `.venv` after a system Python upgrade — silent dep breakage + bad remedy.** System Python jumped 3.12→3.14; the `zkm` uv-tool reinstalled on 3.14 but dev-plugin `.venv`s stayed on 3.12, so `_inject_plugin_venv` (correctly) refuses to inject a 3.12 site-packages into 3.14 → the plugin's top-level `import requests`/etc. crashes. **Two core fixes already landed (2026-06-23):** (1) `_load_plugin_module` registers the module in `sys.modules` before `exec_module` — without it any plugin `@dataclass` crashes on 3.14 (`AttributeError: 'NoneType' … __dict__`); (2) the venv-mismatch warning now prints `uv sync -p python3.14` (plain `uv sync` rebuilds for uv's chosen interpreter, NOT the running one — the original remedy was misleading). **Remaining (this item):** (a) rebuild the still-stale plugin venvs — `claude-ai, claude-code, signal, social, telegram, threema` were on 3.12 as of 2026-06-23 (stt already rebuilt); (b) consider a `zkm plugin doctor`/`zkm plugin sync` that detects venv↔runtime interpreter mismatch across all dev plugins and offers to rebuild with the correct `-p`, so a future interpreter bump is one command, not a per-plugin scavenger hunt. Relates to SB2 (`_inject_plugin_venv`). Filed 2026-06-23. <!-- id:d3a9 -->
