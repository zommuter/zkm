...
- [ ] **N9d (backlog — LLM verifier path).** LLM verifier on residuals. **Trigger FIRED 2026-05-11** (post-N9f re-pilot, N9d-β). Open-set FP classes confirmed: person-lowercase German fragments (1.1% of corpus) + single-token MISC (3.2% suspicious). **Design meeting done 2026-05-12** — see `docs/meeting-notes/2026-05-11-2316-n9d-llm-verifier-design.md`. Section-N] closed-set class fixed by N9c-10. Trigger spec: `docs/meeting-notes/2026-05-11-2218-n9d-n9e-warrant-check.md`. Non-stationarity rider: re-evaluate after any non-mail amender plugin lands.
  ...
  - [ ] **N9d-3.** Scrub integration. Add `--with-verifier` flag to `zkm scrub` CLI in `src/zkm/cli.py`. In `plugins/zkm-ner/convert.py::scrub()._is_scrub_candidate`, add branch: when flag set AND value is `_is_suspicious`-flagged, call `verifier.verify(...)`; treat `"drop"` as candidate-for-removal.
  - [ ] **N9d-4.** Suspicious-predicate port. Move `_is_suspicious` from `plugins/zkm-ner/scripts/pilot.py` into `plugins/zkm-ner/src/zkm_ner/suspicious.py`; pilot.py imports from there. ~10 LOC.
  - [ ] **N9d-5.** Control sample. Implement `--with-verifier-control-pct=1.5` (default 1.5%) on `zkm scrub`. Sample non-suspicious entities at that percentage through the verifier as blind-spot tripwire; log verdicts to stderr.
  - [ ] **N9d-6.** Unit tests in `plugins/zkm-ner/tests/test_verifier.py`. Mock httpx; ≥10 tests covering: Tier 1 yes/no/unclear, Tier 2 escalation, cache hit short-circuit, prompt-hash invalidation, timeout/error → `"unclear"` safe fallback.
  - [ ] **N9d-7.** Stage 1 smoke gate. Run verifier on 5 hand-picked values (3 known-FP + 1 known-legit + 1 ambiguous control); require ≥4/5 match manual expectation.
  - [ ] **N9d-8.** Stage 2 pilot. `zkm scrub ner --with-verifier --dry-run`; dump verdicts to `.zkm-state/ner-verifier-pilot-<ISO8601>.jsonl`; classify 100-value subset into 5 buckets; apply Gate A/B/C.
