...
- [ ] **E9 follow-up: 7c typed-value query probe.** Run `zkm convert ner` cache-bust on the mail corpus (IBAN/amount extractors landed after the last convert run, so those types may not yet be in `entities[]`), then a typed-value probe — searching for a known IBAN should return the source doc via `entities[]`. Contract: `zkm search <known-IBAN>` returns the email that contains it (verifying E8 indexing of `entities[].canonical` on real corpus). See `docs/meeting-notes/2026-05-21-0816-gamma-schema-gap-audit.md` (E9 row). <!-- id:e4fe -->
