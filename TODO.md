...
- [ ] **D1. `docs/restore.md`** — one-pager: backup-first restore procedure; `zkm convert <amender>` + `zkm index` as fallback (~50 min today); cross-link from `CLAUDE.md`. Contract: post-disaster recovery procedure is documented and findable.
- [ ] **D2. Drop dead `.embeddings/` from `store.py:_GITIGNORE`** — one-line cleanup; add regression test asserting `.embeddings/` absent + `.zkm-index/` present. Contract: test fails if dead entry returns.
- [ ] **D3. `zkm verify` backlog entry** — document in `docs/` as a "two-line implementation against ExtractionCache API"; gate = cache-corruption incident OR demand for sub-second integrity check. No build until gate fires.
