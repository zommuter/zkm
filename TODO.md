...
- [ ] **N9d (backlog — LLM verifier path).** LLM verifier on residuals. **Gated on post-N9f re-pilot (see action items below).** Design meeting required before implementation; trigger condition decided 2026-05-11 (see `docs/meeting-notes/2026-05-11-2218-n9d-n9e-warrant-check.md`): re-open design iff (no ≤30-entry phrase list covers ≥80% of FP volume) AND (≥0.5% of total mentions, ~1700 entries on 340k corpus). Also re-evaluate after any non-mail amender plugin (zkm-claude-*) lands.
  - [ ] **N9d-β-1.** Patch `plugins/zkm-ner/scripts/pilot.py` (or `pilot.sh`) so review JSONL output path is timestamped (`ner-pilot-review-YYYYMMDD-HHMM.jsonl`); no overwrite of prior runs. Single-line change; verify by running pilot twice.
  - [ ] **N9d-β-2.** Run `plugins/zkm-ner/scripts/pilot.sh` against current 340k-mention store; capture artefact path.
  - [ ] **N9d-β-3.** Eyeball top-50 per type; manually classify each into 5 buckets: `legit | own-name | boilerplate-legal | closed-set-FP | open-set-FP`; tally per-bucket volume.
  - [ ] **N9d-β-4.** Apply trigger: if (open-set AND ≥0.5% volume) → reopen N9d design meeting with residuals attached; else → close N9d in TODO.md with one-line rationale citing `2026-05-11-2218-n9d-n9e-warrant-check.md`.
