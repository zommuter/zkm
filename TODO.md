...
- [ ] **E9 follow-up (field-test 7c + embed rebuild) — BLOCKED 2026-05-21.** Scope corrected: `zkm convert ner` is **redundant** (mail corpus `entities[]` already carries iban/amount/etc. — verified by histogram + newest-doc read). Only the **dense embed rebuild** is stale (schema 2→3 ⇒ full re-embed of 55506 docs) followed by the 7c probe. **Blocker:** the embed phase consistently 500s after ~27 batches (~864 texts) from `localhost:8080/v1/embeddings`; bge-m3/llama-server appears to enter a transient 500-state for 15–40s under sustained load. v0.7.1 added a 3-attempt retry (5s/10s) but the backoff is too short. **Full investigation + ruled-out list + next steps in `docs/embed-rebuild-500-investigation.md`.** Contract (unchanged): `zkm search "<an-IBAN>" --no-dense` returns the source doc via `entities[]` at high rank. See `docs/field-test-bge-m3.md` step 7c. **Unblock path:** see items below (fde9 resilience bundle, 49c4 stderr, 9e34 branch).
- [x] **[embed] Resilience bundle:** widened embed.py retry to escalating backoff (0/15/30/60s); on stall, logs status + body + origin + text position + wait to stderr; `checkpoint_every` 500→100; bump-and-tag v0.8.0 (commit 5ee4f46). Stall log captures the 161-byte body (D1 diagnostic). — covered by tests (457 pass) on 2026-05-21; **`uv publish` deferred** (no PyPI creds this session). See `docs/meeting-notes/2026-05-21-1011-embed-rebuild-500.md`. <!-- id:fde9 -->
- [ ] **[embed] Capture bge-m3 child stderr:** find where llama-swap pipes child process stderr on zomni; capture at the moment of the 500 (complement to in-band stall log). See `docs/meeting-notes/2026-05-21-1011-embed-rebuild-500.md`. <!-- id:49c4 -->
- [ ] **[embed] Branch on captured body:** server-side (slot/KV/no-slot) → file tracked item in `~/src/zomni/` with body + stall-frequency evidence (candidates: `--parallel`/`-np`, restart/TTL on bge-m3 llama-swap entry, llama.cpp bump); keep OPEN even if backoff unblocks. Client-side → root-cause fix in `embed.py`. See `docs/meeting-notes/2026-05-21-1011-embed-rebuild-500.md`. <!-- id:9e34 -->
