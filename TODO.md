...
- [ ] **W13. Media backfill for existing day-files.** v0.4.0 only CASes media for *newly-ingested* messages: convert skips a day-file when all its `key_id`s already exist (`new_by_key - existing == ∅`), and `_reconstitute` sets `media_path=None`, so already-written placeholders (e.g. ~2500 existing voice notes) do NOT gain media by just setting `media_root` + re-running. Backfill today = lossless regen (whatsapp output is fully derived): `rm -rf $ZKM_STORE/chat/whatsapp` + delete `.zkm-state/zkm-whatsapp.json`, then `zkm convert whatsapp` with `media_root` set. Decide whether to add a targeted `--reingest-media` flag (re-run `_handle_media` over existing manifest entries by re-deriving their source `file_path` from the DB) vs. document the regen recipe. Relates to W6/w6f + `--full-resweep` (id:8d67). <!-- id:4b8e -->
