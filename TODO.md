...
- [ ] [INPUT — meeting] [INBOUND routed:1328 from zomni] HDD/external-drive asset inventory — searchable catalog (zkm search -> which drive); design manifest format (per-drive entity vs find-dump). SCOPED 2026-07-11 → zkm-inventory plugin (id:e65e); + routed:5ea3 from it-infra (formerly zomni; admit rule: descriptive/mutable data → zkm not infra-as-code). See docs/meeting-notes/2026-07-11-2132-inventory-data-scope.md <!-- id:f22d -->
...
- [ ] **zkm-inventory plugin** — SCOPED + HANDED OFF 2026-07-11 → repo plugins/zkm-inventory (origin fievel + public github.com/zommuter/zkm-inventory, v0.1.0 skeleton, 8 RED specs). Tactical build now tracked in that repo's ROADMAP.md: INV1 (id:82a2, drives lane) + INV2 (id:5697, devices lane) dispatchable via /relay executor; lane-c find-dump INV3 (id:46b6) gated fast-follow; git-annex redundancy enrichment dormant (≥2 annex drives). Central retains only the cross-cutting tie: shared push/fetch remote registry (id:998b) + annex boundary. Merges drive lane of id:f22d + device-roster lane of id:d35e. See docs/meeting-notes/2026-07-11-2132-inventory-data-scope.md <!-- id:e65e -->
- [ ] **INV3a (core; zkm-inventory find-dump prereq) — dense-leg per-path opt-out.** Add a config `dense_skip_prefixes` to `src/zkm/embed.py` (+ `config.py`) so the dense/embeddings leg skips docs whose rel_path starts with a configured prefix (default incl. `inventory/find-dump/`); BM25 unchanged. RED: index a store with a normal doc + an `inventory/find-dump/…` doc → BM25 index has BOTH rel_paths, but `EmbedStore.paths` includes the normal doc and NOT the find-dump one; `zkm index` still graceful with no embed endpoint. Central (edits src/zkm/**, boundary rule #1). Independent of the HDD-content pilot → dispatchable now; unblocks zkm-inventory INV3b (find-dump md shards must not blow the embed index). See plugins/zkm-inventory/docs/inv3-lane-c-design.md §INV3a. <!-- id:8fb4 -->
