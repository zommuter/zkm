...
- [ ] Scope + build **zkm-inventory plugin** — THREE lanes: (a) DRIVES descriptive map — per-drive md, manifest-driven; OPTIONAL git-annex whereis/info redundancy enrichment as a DORMANT seam for the annex-managed subset only, gated on ≥2 annex-managed drives (0 today); (b) HARDWARE (RPi/IoT) roster — pure hand-authored descriptive md (model/location/in-use/dust-status), NO git-annex ever (a device is not annexed content); (c) DRIVE-CONTENTS find-dump index (FAST-FOLLOW after v1) — mount each drive + record file listing (paths/sizes/mtimes) → searchable per-drive content manifest so `zkm search "<title>"` → which drive; git-annex-INDEPENDENT (covers the bulk non-annex content annex-whereis never sees; complementary to lane-a's annex seam); heaviest lane (large manifests → per-drive chunking/compact format; mount orchestration; re-sweep = git diff = when a file moved/vanished); demonstrated need = locate favorite-movie/media across drives (2026-07-11). v1 = lanes (a)+(b) manifest-driven, buildable TODAY at 0 annex drives (the ≥2-annex-drives gate applies to lane-a enrichment seam ONLY, not the plugin/hardware/find-dump). Typed entities[] (drive, device), indexed & searchable; reads shared push/fetch remote registry (id:998b, routed:12fc); own repo + new-plugin dispatch. Merges drive lane of id:f22d + device-roster lane of id:d35e. See docs/meeting-notes/2026-07-11-2132-inventory-data-scope.md <!-- id:e65e -->
