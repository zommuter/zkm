...
- [x] **(core) Implement D2 — unified `zkm push`.** SHIPPED 2026-07-12 (`push_store` one-directional durability push: `git annex copy --to <remote>` for content + `git annex sync --no-pull --no-content <remote>` for refs; content-by-default, `--no-content` for refs-only; `_resolve_push_remote()` default-remote helper). NO index-sync leg (D3 shipped: `embeddings.npz` rides the annex copy, `bm25.pkl` regenerates). `--fast-seed` DEFERRED (observe-before-preventing, N=1 cold seed; additive flag). Stale-premise corrected: the push twin is `zkm pull` (git remotes ARE the registry), NOT `zkm fetch`. See 2026-07-12-1030-d2-unified-zkm-push note. <!-- id:998b -->
