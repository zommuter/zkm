...
- [ ] **SOC1.** Build `zkm-social` plugin skeleton: `plugin.yaml` (`creates_dirs: [contacts, originals/contacts]`), `convert(store_path, config) -> list[Path]` with per-network parser dispatch. Contract: configured source dir + network config routes to the right parser; emits vCard-compatible contact doc shape. See `docs/meeting-notes/2026-06-06-1656-social-network-profile-scraping-scope.md`. <!-- id:56ac -->
- [ ] **SOC2.** GitHub parser module (lane B — public API): fetch `api.github.com/users/<login>`, map login/name/bio/company/location/blog/avatar → `contacts/<slug>.md`, typed `entities[]` at `scope:profile.github`, avatar → CAS, dedup-keyed on profile URL. Contract: GitHub username → one contact note; re-run is git no-op. See `docs/meeting-notes/2026-06-06-1656-social-network-profile-scraping-scope.md`. <!-- id:017f -->
- [ ] **SOC3.** LinkedIn parser module (lane A — browser-save): parse a browser-saved LinkedIn profile (HTML/PDF/MHTML) for name/headline/current-employer/location/photo/profile-URL; emit `contacts/<slug>.md` at `scope:profile.linkedin`, photo → CAS, dedup-keyed on normalized profile URL. Contract: saved profile file → one contact note; no live network I/O, no credentials. See `docs/meeting-notes/2026-06-06-1656-social-network-profile-scraping-scope.md`. <!-- id:7f55 -->
