...
- [ ] **N9c-html: HTML-entity scrub pattern** — `&gt;&nbsp;` and similar runs extracted as ORG because zkm-eml renders HTML bodies with entities undecoded before NER. Not caught by `_BASE64_FRAGMENT_RE`. Fix: new `drop_html_entity_artefacts()` pattern in `zkm_ner/filters.py` (regex matching `^[&;a-z0-9\s#]+$` fragments of ≤30 chars) OR `html.unescape()` call at render time in zkm-eml. N=2: blocks embed re-run cleanliness + future roundtrip test coverage. See `docs/meeting-notes/2026-05-29-0859-embed-oversized-chunk-timeout.md`.
