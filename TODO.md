...
- [ ] Scope + build **zkm-inventory plugin** — converter from a hand-authored manifest → per-drive + per-device md with typed entities[] (drive, device), indexed & searchable; git-annex whereis/info redundancy enrichment as a DORMANT seam gated on ≥2 annex-managed drives (0 today); reads the shared push/fetch remote registry (id:998b, routed:12fc); go/no-go first check = count annex-managed drives; own repo + new-plugin dispatch. Merges the drive lane of id:f22d + device-roster lane of id:d35e. See docs/meeting-notes/2026-07-11-2132-inventory-data-scope.md <!-- id:e65e -->
