...
- [ ] **N9c.** spaCy common-noun false-positive gating. Design decided 2026-05-11 (see `docs/meeting-notes/2026-05-11-0946-ner-next-after-n9b.md`). Mechanism: hybrid POS-filter (`ent.root.pos_ == "PROPN"`) + `_COMMONNOUN_STOPLIST` for PROPN-tagged abbreviations. Applied to all spaCy NER types; pattern-overlay entities bypass.
  - [ ] **N9c-1.** `_pos_filter(ent)` in `plugins/zkm-ner/src/zkm_ner/extract.py` — applied after `nlp(text)`, before pattern merge. 6 tests in `plugins/zkm-ner/tests/test_pos_filter.py`.
  - [ ] **N9c-2.** `_COMMONNOUN_STOPLIST` = {Du, wünschen, Zeit, EUR, CHF, UTC, MESZ, CEST, Internet, CV, AGB, HRB} + `drop_commonnoun_stoplist` in `plugins/zkm-ner/src/zkm_ner/textfilter.py`. Parametrised tests added to `tests/test_textfilter.py`.
  - [ ] **N9c-3.** Bump `model_version` in `plugins/zkm-ner/src/zkm_ner/version.py` to `+textfilter-v1+posfilter-v1` for cache invalidation.
  - [ ] **N9c-4.** Extend zkm-ner `scrub()` in `plugins/zkm-ner/convert.py` — `_COMMONNOUN_STOPLIST` predicate (cheap) + isolated-POS-tag predicate (principled). 3 tests in `plugins/zkm-ner/tests/test_scrub.py`.
  ...
  - [ ] **N9c-7.** Add N9d + N9e backlog entries to TODO.md (this item).
...
- [ ] **Meeting: plugin name convention for `zkm convert`** — `zkm convert ner` works (strips `zkm-` prefix) but `zkm convert zkm-eml` is the established form for eml. Inconsistent: does the CLI accept both forms for all plugins, or is `ner` a coincidence? Decide: (a) always require full `zkm-<name>`, (b) always accept bare `<name>` and strip prefix internally, (c) accept both with a deprecation warning. Previously touched but not resolved.
