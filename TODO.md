...
- [ ] **N9e (backlog).** Closed-loop verifier denylist — append-only JSONL at `<store>/.zkm-state/ner-verifier-denylist.jsonl`; one record per `(value, type)`: `{value, type, verdict, source, model_version, first_seen, heuristic_would, n_observations}`. `source ∈ {verifier, heuristic, manual}`; `verdict ∈ {drop, keep}` (drops-only direction designed; keeps-becoming-sticky deferred — precedence ambiguity). **Gate: (N9d shipped) AND (≥5 verifier-override cases observed in Stage 2 pilot).** Schema sketch in `docs/ner.md` (N9d-11 action item). Conflict-resolution for allow+deny overlap unresolved — design meeting before implementation.
...
- [ ] **N9d (in progress — LLM verifier path).** LLM verifier on residuals. **Trigger FIRED 2026-05-11** (post-N9f re-pilot, N9d-β). Open-set FP classes confirmed: person-lowercase German fragments (1.1% of corpus) + single-token MISC (3.2% suspicious). **Design meeting done 2026-05-12** — see `docs/meeting-notes/2026-05-11-2316-n9d-llm-verifier-design.md`. **N9d-1 through N9d-6 implemented 2026-05-12** (verifier.py, suspicious.py, cli + scrub integration, 16 tests). Remaining: N9d-7 through N9d-12 (Stage 1 gate, Stage 2 pilot, docs). Section-N] closed-set class fixed by N9c-10. Trigger spec: `docs/meeting-notes/2026-05-11-2218-n9d-n9e-warrant-check.md`. Non-stationarity rider: re-evaluate after any non-mail amender plugin lands.
  ...
  - [ ] **N9d-8.** Stage 2 pilot. `zkm scrub ner --with-verifier --dry-run`; dump verdicts to `.zkm-state/ner-verifier-pilot-<ISO8601>.jsonl`; classify 100-value subset into 5 buckets; apply Gate A/B/C. **Human-in-the-loop required**: the 5-bucket classification must be done by the user, not by Claude — LLM self-evaluation of its own verdicts is unreliable (smoke gate showed model confidence ≠ correctness for borderline cases). **Pilot-dump implemented** (zkm-ner v0.7.0, 2026-05-12): `--with-verifier --dry-run` auto-writes JSONL to `.zkm-state/ner-verifier-pilot-YYYYMMDD-HHMM.jsonl`. **Next: run the pilot** (aya-expanse-8b cold-start expected; existing cache will hit for previously-seen values).
  - [ ] **N9d-9.** Per-language accuracy lens. Tag each verdict with langdetect language; compute DE/EN per-language correct-drop / FP-drop-of-legit; record in pilot note (not blocking).
  - [ ] **N9d-10.** Gate decision artefact. Write `docs/meeting-notes/YYYY-MM-DD-HHMM-n9d-pilot-results.md`. Gate A (FP-drop-of-legit ≤2% AND correct-drop ≥60%) → proceed to `--apply`; Gate B (2–5% or <60%) → retry; Gate C (≥5%) → close N9d.
  - [ ] **N9d-11.** N9e sketch into `docs/ner.md` — subsection "N9e — closed-loop verifier denylist (deferred)" with schema example + gate condition. Half-page max.
  - [ ] **N9d-12.** TODO.md updates once N9d ships: mark N9d top-level as done; update N9e entry with observed override-case count vs. gate.
...
- [ ] **`zkm status --follow` / watch mode** — periodically re-send SIGUSR1 + re-read PID files and redraw the table in-place (ANSI cursor-up). `--follow` loops until all processes exit; optional `--leave-if-done` exits as soon as the table is empty (useful for scripting: `zkm status --follow --leave-if-done && zkm index`). Poll interval default ~2s. Previously deferred in the SIGUSR1 meeting (N<2 background callers at the time); now relevant given background scrub runs.
- [ ] **`zkm status` ETA column** — Add optional `eta_seconds` field to PID file schema; `RunSession.tick()` accepts an optional `eta_seconds` kwarg so callers that already have an ETA (e.g. tqdm's `format_dict["remaining_s"]` in convert/scrub) can forward it directly instead of recomputing. Fallback: derive from `elapsed / current * (total - current)` when `eta_seconds` is absent. SIGUSR1 handler should include ETA in its stderr dd-style line too. Display as `ETA ~Xm` in status table; suppress when unknown or `current == 0`.
- [ ] **Scrub continuability / resume-on-restart** — `zkm scrub` has no watermark; killed runs restart from file 0 (unlike convert/index which have git-commit watermarks). Two parts: (a) **pilot JSONL incremental flush** — write each record to `pilot_dump_path` immediately after the verifier call instead of accumulating in memory; on restart, skip files already in the dump (keyed by `file` field). (b) **scrub watermark** — persist last-processed file path to `.zkm-state/scrub-<plugin>-watermark.json`; resume skips already-processed files. Note: verifier cache (ExtractionCache) already persists each verdict atomically — LLM work is never lost on kill, only the pilot JSONL and file-walk progress are.
