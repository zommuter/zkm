...
- [ ] **Concurrent-run guard** *(design decided 2026-05-14, see `docs/meeting-notes/2026-05-14-1011-concurrent-run-guard.md`)* — v1 scope: `convert`/`scrub`/`index` only. Precheck inside `RunSession.__enter__` calls `_take_status_snapshot(send_sigusr1=False)`; conflict matrix = same-key always + cross-key within `{convert, scrub}` (sidecar race). Raise `ConcurrentRunError(ClickException)` with `exit_code=75` (`EX_TEMPFAIL`). Bypass via `ZKM_BYPASS_RUN_GUARD=1`. TOCTOU accepted; observability via new `zkm doctor` check for multiple live PID files sharing `(command, args[0])`. Other store-mutating commands stay unguarded (git's own `.git/index.lock` covers them).
  - [ ] `src/zkm/runstate.py` — add `ConcurrentRunError`, `_conflicts_with(other_row)` helper, precheck in `__enter__`; bypass env var read.
  - [ ] `src/zkm/cli.py` — add `_count_concurrent_keys(rows)` helper; new `zkm doctor` line for "concurrent same-key PID files detected" when count > 1 per key.
  - [ ] `tests/test_runstate.py` — same-key collision exit 75, cross-key `{convert, scrub}` raises, `convert × index` allowed, `index × index` raises, bypass env, stale-PID cleanup unaffected, error message includes other pid + start time.
  - [ ] `docs/install.md` (or new `docs/concurrent-runs.md`) — document guard, bypass, exit-75 convention, mbsync-hook implication (`|| true` or `zkm status --wait; zkm convert eml`).
  - [ ] `CLAUDE.md` — pointer to docs + `ZKM_BYPASS_RUN_GUARD` env var alongside `ZKM_BYPASS_DIRTY_CHECK`.

**Spawned follow-ups (from 2026-05-14 concurrent-run-guard meeting):**

- [ ] **Sidecar atomicity — `fcntl`-lock `merge_producer`** — verified race in `src/zkm/sidecar.py:21-48`: read → modify → atomic-write across processes loses producer entries (P_A reads [X], P_B reads [X], P_A writes [X,A], P_B writes [X,B] — A lost). Reference flock pattern: `~/.claude/skills/git-diary-workflow/git-lock-push.sh` (30 s timeout). Once shipped, the broader `convert × convert(any args)` refusal in the v1 guard could narrow to `convert × scrub` only — recommend keeping broad (one line, cheap insurance).
