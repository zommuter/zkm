...
- [ ] **zkm-eml: inline data-URI images → CAS detach (prerequisite for [402c]).** Before markdownify in `plugins/zkm-eml/.../convert.py` (where `store_path`/`msg`/`dest`/`inbox` in scope): parse `msg.html_body`, extract `<img src="data:<mediatype>;base64,...">`, decode to bytes, `write_object(store_path,"mail",bytes)`, synthesize `ParsedAttachment` (`filename=inline-<sha8>.<ext>`, `part_index=-1`, `is_inline=True`), feed existing `write_original`/`symlink_with_sidecar`/`_att_entry`, drop the tag so markdownify emits a clean body. Test: fixture HTML email with data-URI img → CAS object + frontmatter `attachments[]` entry + no `data:` in body. Bump-and-tag zkm-eml minor; `uv publish` deferred. See `docs/meeting-notes/2026-05-21-1908-embed-rebuild-eta-decision.md`. <!-- id:15b2 -->
- [ ] **GPU contention probe before the clean rebuild.** `nvidia-smi dmon` during a live bge-m3 embed batch on zomni; if baseline is contention-bound vs always-on gemma4-e4b, pause gemma4-e4b in llama-swap for the rebuild window. Record result (one line) in `docs/embed-rebuild-500-investigation.md`. See `docs/meeting-notes/2026-05-21-1908-embed-rebuild-eta-decision.md`. <!-- id:88f8 -->
- [ ] **[embed] Resume + complete the schema-3 embed rebuild (gated on [15b2]).** After zkm-eml detach lands, re-run `zkm convert zkm-eml` and verify only ~15 data-URI files get rewritten (mtime churn small). Then run the GPU probe [88f8] and one clean schema-3 `zkm index` to completion (resumes 1796 checkpoint; re-embeds the re-converted files; backgrounded, checkpoints every 100). Decision (2026-05-21): feature-first single clean pass; see `docs/meeting-notes/2026-05-21-1908-embed-rebuild-eta-decision.md`. <!-- id:402c -->
- [ ] **[embed] Run field-test 7c typed-value probe + close E9.** After rebuild completes (`zkm doctor` embed docs == md count, schema 3): derive a real IBAN prefix from an `entities[].type: iban` value, `zkm search "<prefix>" --no-dense -k 5`, verify top hit has IBAN in `entities[]` not body; repeat for `amount`. Record in `docs/field-test-bge-m3.md` step 7. <!-- id:2c6e --> <!-- id:9e34 -->
- [ ] **`zkm scrub zkm-eml` over the ~14 base64-bloated-frontmatter files.** Removes existing base64-derived garbage `value:` entities that set-union merge cannot drop. Run after [15b2] lands (so re-convert doesn't re-introduce them via a fresh amender pass on the same emails). Independent of the detach PR scope. See `docs/meeting-notes/2026-05-21-1908-embed-rebuild-eta-decision.md`. <!-- id:541f -->
