...
- [ ] **LLM conflates generic "Rechnung" docs with "Stromrechnung"** — step-5 query "Wie hoch war meine letzte Stromrechnung?" returned O2 phone bill figures; likely no Stadtwerke electricity bill emails in corpus (corpus gap) OR retrieval returns too many topically adjacent billing docs; investigate: check if any electricity utility emails exist, consider adding entity/tag filtering to `zkm query` for bill-type disambiguation
