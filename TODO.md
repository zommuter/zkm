...
- [ ] **`zkm status` ETA column** — PID file already has `started_at`, `current`, `total`; compute `elapsed / current * (total - current)` and display as `ETA ~Xm` in the status table. Suppress when `total` is unknown or `current == 0`. Class 1 (no design needed).
