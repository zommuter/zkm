...
- [ ] **(store+core) Implement D1 — contested artifact classes → T1 git.** DECIDED 2026-06-24 (storage-tiers note). (a) CAS-json sidecars → git: add `**/_objects/**.json annex.largefiles=nothing` to the `store.py` gitattributes template **and** live `~/knowledge/.gitattributes` (keep `**/_objects/**`=anything for binaries; mixed git+annex in `_objects/` accepted). (b) `.amendments.json` → git: remove `*.amendments.json` from `.gitignore`; track ALL consistently (incl. the ~46k grandfathered — currently half-tracked). Rationale: per-producer `producer_sets` only regenerable by re-running NER (>2h) → can't be regenerate-tier; small text → git. Migration: existing CAS-json move annex→git (re-add). RED: `check-attr` on `_objects/*.json` = `nothing` while `_objects/<binary-hash>` = `anything`. See 2026-06-24-1350-storage-tiers-restore-sync note. <!-- id:6c4f -->
