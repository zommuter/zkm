...
- [ ] Scope + build **zkm-inventory plugin** — TWO independent asset lanes: (a) DRIVES — per-drive md, manifest-driven; OPTIONAL git-annex whereis/info redundancy enrichment as a DORMANT seam for the annex-managed subset only, gated on ≥2 annex-managed drives (0 today); (b) HARDWARE (RPi/IoT) roster — pure hand-authored descriptive md (model/location/in-use/dust-status), NO git-annex involvement ever (a device is not annexed content). Both lanes are buildable TODAY with 0 annex drives — the ≥2-annex-drives gate applies to the drive-lane enrichment seam ONLY, NOT to the plugin or the hardware roster. Typed entities[] (drive, device), indexed & searchable; reads the shared push/fetch remote registry (id:998b, routed:12fc); own repo + new-plugin dispatch. Merges the drive lane of id:f22d + device-roster lane of id:d35e. See docs/meeting-notes/2026-07-11-2132-inventory-data-scope.md <!-- id:e65e -->
