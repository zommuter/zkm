...
- [ ] **N9c.** spaCy common-noun false-positive gating. Design decided 2026-05-11 (see `docs/meeting-notes/2026-05-11-0946-ner-next-after-n9b.md`). Mechanism: hybrid POS-filter (`ent.root.pos_ == "PROPN"`) + `_COMMONNOUN_STOPLIST` for PROPN-tagged abbreviations. Applied to all spaCy NER types; pattern-overlay entities bypass.
  ...
  - [x] **N9c-6.** Re-run `plugins/zkm-ner/scripts/pilot.sh`; compare person/org top-N; target <5% legit-ORG loss vs post-N9b baseline. Document delta here — 2026-05-11.
    **Delta vs post-N9b (719,504 mentions):** 471,892 total (-34.4%, -247,612). Legit-ORG target MET: Google LLC ×3204, PayPal ×1892, Amazon WS ×1074, SBB ×542, ETH ×485 all intact. Person top is now Tobias Kienzler ×11,269.
...
- [ ] **N9c-8 (backlog).** Pipe-cell artifact filter — post-extraction value filter: reject entities whose value matches `^[\s|]+$` (pure pipe/whitespace, not caught by N9b row-level strip because they come from within data rows). Add to `drop_commonnoun_stoplist` or as a separate `drop_structural_artefacts` in `textfilter.py`. Bump `model_version`, extend `scrub()`, re-pilot. N=2: pipe-cell FPs + future HTML-table artefacts.
- [ ] **N9c-9 (backlog).** Bilingual isolated-POS in scrub — `_isolated_pos` in `convert.py:scrub()` currently only loads `de_core_news_sm`; English common nouns ('Learn', 'Link', 'Actions', 'Download') are tagged PROPN/X by the German model and survive. Fix: when DE model returns PROPN for a value with no DE-typical characters (no umlauts, or langdetect says EN), retry with EN model. Or: load DE + EN and take the stricter result.
...
- [ ] **`zkm scrub` has no progress indicator** — `cmd_scrub` in `cli.py` does not wire tqdm or `RunSession`; scrub on a 55k-file store runs silent for several minutes. Fix: add tqdm progress bar (same pattern as `cmd_convert`) and optionally a `RunSession` so `zkm status` can observe it. N=2: ner scrub + future POS-filter scrub. Impl-ready.
- [ ] **`zkm convert` reports "Converted 0 file(s)" for amenders** — amenders return `[]` by contract so the count is technically correct, but the message is misleading. Fix: if `plugin.kind == "amender"`, suppress the "Converted N file(s)" line or replace with "Amended via <plugin>" (amendment counts already printed by the plugin itself). Impl-ready.
- [ ] **`zkm status` CMD column hides plugin** — `cmd_status` displays `row["command"]` ("convert") but ignores `row["args"]` (already stored in PID file, e.g. `["ner"]`). During mbsync hook runs, "convert" is ambiguous: could be the eml convert or the ner amender (which runs as a sub-step without its own RunSession). Fix: display `command(args[0])` in the CMD column — e.g. "convert(ner)" — one-line change in `cli.py:819`. No schema change needed.
- [ ] **Meeting: plugin name convention + verb order** — two open questions for the same meeting: (a) name form: `zkm convert ner` works (strips `zkm-` prefix) but `zkm convert zkm-eml` is the established form — accept both, or normalise? (b) verb order: `zkm convert <plugin>` vs `zkm <plugin> convert` / `zkm <plugin> run` — the latter matches git-plugin style and disambiguates status display. Previously touched but not resolved; add verb-order question from 2026-05-11 status observation.
- [ ] **Hook output swallowed by journald rate-limiting** — `systemd-cat` burst limit (28 lines) cut off amender output and commit messages during hook runs with many files. Fix: add `--ratelimit-interval 0` to the `systemd-cat` call in `plugins/zkm-eml/hooks/post-commit`, or write to a log file under `.zkm-state/` instead.
