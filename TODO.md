...
- [ ] **E7.** Second-round value-type extractors (one session each): ~~`iban`~~ ✓ (ISO 13616 + mod-97 checksum — 16 tests, zkm-ner v0.10.0, 2026-05-12), `email` (γ-migration verify), `phone` (E.164 + national), `url` (RFC 3986 shape), `invoice_id` (common format regex), `tracking_id` (DHL/Post/UPS), `registration_code` (HRB, DIN, ISBN, EAN13).
