...
- [ ] **`docs/object-storage.md` extraction-cache section stale** — the "Extraction cache (design only — Phase 3+)" section describes a per-CAS-object content cache for pdf/photo/scan OCR, which is still design-only. However `src/zkm/extraction_cache.py` (the per-document NER extraction cache keyed by body_sha256) shipped in Phase 2.5 and is a distinct design. The two caches should be clearly differentiated in `object-storage.md` with a pointer to `docs/ner.md`. Low priority docs hygiene; no code change.
