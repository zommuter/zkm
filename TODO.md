...
- [ ] **[embed] Resilience bundle:** widen embed.py retry to escalating backoff spanning ≥40s (0/15/30/60s); on stall, log status + body + origin + text position + wait to stderr; lower `checkpoint_every`; bump-and-tag v0.8.0, `uv publish`. Stall log captures the 161-byte body (D1 diagnostic) — one run diagnoses AND attempts the unblock. See `docs/meeting-notes/2026-05-21-1011-embed-rebuild-500.md`. <!-- id:fde9 -->
