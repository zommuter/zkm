...
- [ ] **INV3a (core; zkm-inventory find-dump prereq) — dense+amender per-path opt-out.** (1) `dense_skip_prefixes` in `src/zkm/embed.py` (+ `config.py`) so the dense/embeddings leg skips docs whose rel_path starts with a configured prefix (default incl. `inventory/find-dump/`); (2) SIBLING `amender_skip_prefixes` so `zkm convert`'s amender leg (id:63bb created-list) does NOT pass find-dump shards to zkm-ner (millions of filename tokens → cost + garbage entities). BM25 unchanged. RED: dense `EmbedStore.paths` excludes the find-dump doc; a convert with amenders does NOT amend a find-dump shard; `zkm index` still graceful with no embed endpoint. Central (edits src/zkm/**). Dispatchable now; unblocks zkm-inventory INV3b. See plugins/zkm-inventory/docs/inv3-lane-c-design.md §Q5/§INV3a + Fable review #7. <!-- id:8fb4 -->
