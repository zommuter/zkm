...
- [ ] **Plugin-specific deps when loaded via importlib** — when `zkm convert` loads a plugin via `importlib.util.spec_from_file_location` into the main process, the plugin runs in the main zkm venv which lacks plugin-only deps (e.g. `ftfy`, `charset-normalizer` in zkm-eml). Current workaround: each plugin self-injects `.venv/lib/python*/site-packages` into `sys.path` at import time — but only **eml** does this (`convert.py:14-19`); **pdf/scan/vcard** import their deps (pypdf/pytesseract/vobject) at module top-level *without* injecting first, so loading them into the core venv fails (surfaced by `zkm test` 2026-06-01; conformance.py works around it with a generic `_inject_plugin_venv`, but `zkm convert pdf` from the core venv would still fail). Explore proper solutions: (a) subprocess isolation per plugin, (b) uv-run-in-plugin-venv wrapper, (c) declare plugin deps as optional extras in core and install them together, (d) move eml's self-injection idiom into core's `_load_plugin_module` so all plugins benefit uniformly. Warrants a scoping meeting before changing the plugin loading model.
...
- [ ] **SB2 (core).** `src/zkm/convert.py:405-416` `_load_plugin_module` — lift eml's `.venv/site-packages` self-injection into core so all dev-symlink plugins get it. Contract test: `zkm convert pdf` no longer fails on `pypdf` import from core venv. See meeting note. <!-- id:19ea -->
...
- [ ] **SB4 (eml pilot).** `plugins/zkm-eml/` — repackage into importable `zkm_eml` package; ship `plugin.yaml`+`convert.py` as package data; declare `[project.entry-points."zkm.plugins"]`; bump+tag+publish real wheel replacing 0.0.1 stub. Contract: `uv tool install zkm --with zkm-eml` → `zkm convert eml`; roundtrip corpus + conformance pass. See meeting note. <!-- id:4d01 -->
- [ ] **SB5 (rollout).** Repackage + entry-point + publish remaining 5 real wheels (ner, pdf, photo, scan, notmuch) mirroring eml pilot. Gated on SB4. See meeting note. <!-- id:42ba -->
