...
- [x] [INPUT — meeting] HDD/external-drive asset inventory — SUPERSEDED by id:e65e (zkm-inventory plugin; drive lane shipped v0.2.0). routed:1328 (zomni) + routed:5ea3 (it-infra) both folded. Closed 2026-07-11 (Fable review #1: was subsumed by e65e). <!-- id:f22d -->
...
- [x] **INV3a (core; zkm-inventory find-dump prereq) — dense+amender per-path opt-out.** Shipped 2026-07-11: `dense_skip_prefixes` (`src/zkm/embed.py::build_embed_store` + `config.py` default `["inventory/find-dump/"]`) filters docs out of the dense leg only; SIBLING `amender_skip_prefixes` (`src/zkm/cli.py` `cmd_convert` amender-dispatch loop + `config.py` default `["inventory/find-dump/"]`) filters the `created` list (id:63bb) before it reaches any amender. BM25 unchanged (verified by guard test). Shared prefix-match helper `zkm.config.path_has_skip_prefix` (leading-`./`-robust). 7 new RED→GREEN tests (`tests/test_embed.py`, `tests/test_cli_amender_skip_prefixes.py`); full suite 623 passed. No version bump / uv.lock touch (batched separately). Unblocks zkm-inventory INV3b. See plugins/zkm-inventory/docs/inv3-lane-c-design.md §Q5/§INV3a + Fable review #7. <!-- id:8fb4 -->
