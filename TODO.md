...
- [ ] **N9d (backlog — LLM verifier path).** LLM verifier on residuals. **Trigger FIRED 2026-05-11** (post-N9f re-pilot, N9d-β). Open-set FP classes confirmed: person-lowercase German fragments (1.1% of corpus) + single-token MISC (3.2% suspicious). Design meeting required before implementation. New closed-set class also found: `Section N]` markdown link-target artifacts (~571 occurrences) — may be fixable with textfilter before N9d design meeting. Trigger spec: `docs/meeting-notes/2026-05-11-2218-n9d-n9e-warrant-check.md`. Non-stationarity rider: re-evaluate after any non-mail amender plugin lands.
