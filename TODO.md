...
- [ ] **[embed] Resume + complete the schema-3 embed rebuild.** `zkm index` resumes from the 1796-doc checkpoint (schema 3). Run backgrounded (resumable, checkpoints every 100). **OPEN: ETA projected ~19.5h** at pause due to dense email chunks (HTML/base64/quoted-printable, 75–100s/batch). Decide: (a) let it run, (b) chunking-quality cleanup to drop dense cruft (also improves retrieval), (c) check GPU throughput/contention with always-on gemma4-e4b. NOT a lever: `_EMBED_BATCH` (same total tokens). See `docs/embed-rebuild-500-investigation.md` "OPEN DECISION". <!-- id:402c -->
