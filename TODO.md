...
- [ ] **LLM hallucinates when corpus lacks the queried document type** — live test: "Stromrechnung" retrieves O2 phone invoices (keyword "Rechnung" overlaps); LLM confidently invents an electricity bill from those. Fix candidates: (a) add a "no relevant documents found" escape hatch in the answer prompt when top-hit scores are low, (b) relevance threshold before passing docs to context, (c) instruct LLM to state explicitly when retrieved docs don't match the question
