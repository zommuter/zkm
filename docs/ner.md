...
| Pattern type | `entity.type` | Notes |
|---|---|---|
| Email address | `email` | RFC 5322 local+domain |
| Phone — DE/CH | `phone` | +41 / +49 / 0800 / 07xx formats |
| URL | `url` | http/https; excludes bare domains |
...
| Org gazetteer | `org` | canonical lookup via `gazetteers/orgs.yaml` |

spaCy types that pass the POS-filter: `PER` (→ `person`), `ORG` (→ `org`), `LOC` (→ `place`), `MISC` (→ `misc`).
