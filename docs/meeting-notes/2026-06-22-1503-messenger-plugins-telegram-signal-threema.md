...
- [ ] zkm-telegram: implement `convert()` parsing Telegram Desktop `result.json` → per-chat-day transcripts; `message_id = telegram:<chat_id>:<msg.id>`, reply graph from `reply_to_message_id`, media → CAS. No decryption. Ship conformance fixture. (session 2026-06-22-1503) <!-- id:849f -->
...
- [ ] Reserve prefix letters T/G/H in CLAUDE.md TODO-prefix table commentary; allocate a full row per plugin once it hits ≥3 unchecked items. (session 2026-06-22-1503) <!-- id:8cf8 -->
